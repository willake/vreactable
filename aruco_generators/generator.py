...
def packImages(
    ...
    for arucoFile in arucoFiles:
        ...
        if y + arucoImg.size[1] + gapSize * 2 > a4Height:
            ...
            y = gapSize
            draw.line(
                [(x - gapSize, 0), (x - gapSize, a4Height)],
                fill=GRID_COLOR,
                width=GRID_BORDER_WIDTH,
            )
            draw.line(
                [
                    ...
                    (x + arucoImg.size[1] + gapSize, a4Height),
                ],
                fill=GRID_COLOR,
                width=GRID_BORDER_WIDTH,
            )
...
def generatePackedArucoMarkers(
    markerFolder, packedFolder, arucoDict, numMarkers, markerSizecm, gapSizecm
):
